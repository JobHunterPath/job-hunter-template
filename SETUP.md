...
| JobSpy returns very few jobs | python-jobspy is unavailable, JobSpy is disabled, or the searched boards had few matches | Confirm the core image includes python-jobspy and `http.job_boards.jobspy.enabled: true` in `config/api_config.yml`. Increase `hours_old` to broaden the time window. |
| JSearch returns zero jobs | `RAPIDAPI_KEY` is absent or exhausted, or RapidAPI/JSearch returned no matches | Confirm `RAPIDAPI_KEY` is set as a GitHub secret and `http.job_boards.jsearch.enabled: true` in `config/api_config.yml`. |

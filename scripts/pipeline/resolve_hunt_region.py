"""Resolve which search_config region a scheduled hunt slot should run."""
...
DEFAULT_CONFIG_PATH = ROOT / "config" / "search_config.yml"
        ...
        if slot == 0:
            ...
            if secondary_idx >= len(secondary_regions):
                return 0, {
                    "should_run": "false",
                    "reason": (
                        f"No secondary region configured for hunt slot {slot} "
                        f"(schedule {event_schedule!r})"
                    ),
                }

...
import pytest
from pipeline import cover_writer
...
def test_write_cover_creates_markdown(tmp_path):
    with patch('pipeline.cover_writer.get_llm_client', return_value=_mock_client(BODY)):
        md_path = cover_writer.write_cover(MATCH, str(tmp_path), CONFIG)

    assert os.path.exists(md_path)


def test_write_cover_markdown_contains_company(tmp_path):
    with patch('pipeline.cover_writer.get_llm_client', return_value=_mock_client(BODY)):
        md_path = cover_writer.write_cover(MATCH, str(tmp_path), CONFIG)
...
def test_write_cover_returns_md_path(tmp_path):
    with patch('pipeline.cover_writer.get_llm_client', return_value=_mock_client(BODY)):
        md_path = cover_writer.write_cover(MATCH, str(tmp_path), CONFIG)
...
def test_write_cover_api_error_raises(tmp_path):
    mock = MagicMock()
    mock.complete.side_effect = Exception('API down')
    with patch('pipeline.cover_writer.get_llm_client', return_value=mock):
        with pytest.raises(Exception, match='API down'):
            cover_writer.write_cover(MATCH, str(tmp_path), CONFIG)


def test_write_cover_no_forbidden_phrases(tmp_path):
    forbidden = [
        'I am passionate',
...
def test_write_cover_no_story_id_citations(tmp_path):
    body_with_citation = BODY + " Core platform components [STORY-01] drive adoption."
    with patch('pipeline.cover_writer.get_llm_client', return_value=_mock_client(body_with_citation)):
        md_path = cover_writer.write_cover(MATCH, str(tmp_path), CONFIG)
